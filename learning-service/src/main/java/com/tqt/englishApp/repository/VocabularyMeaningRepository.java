...
public interface VocabularyMeaningRepository extends JpaRepository<VocabularyMeaning, Integer> {
    @Query(value = "SELECT * FROM vocabulary_meaning ORDER BY RAND() LIMIT :limit", nativeQuery = true)
    List<VocabularyMeaning> findRandomDistractorsPool(@Param("limit") int limit);

    @Query("SELECT vm FROM VocabularyMeaning vm " +
           "JOIN FETCH vm.vocabulary v " +
           "JOIN v.subTopics st " +
