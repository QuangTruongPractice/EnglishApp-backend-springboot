...
public class SubTopicControllerTest {
    ...
    void listSubTopics_WithoutParams_Success() throws Exception {
        Page<SubTopicResponse> page = new PageImpl<>(Collections.emptyList(), PageRequest.of(0, 10), 0);
    ...
    void subTopicsForm_Success() throws Exception {
        List<MainTopicResponse> mainTopics = Collections.singletonList(new MainTopicResponse());
    ...
    void updateSubTopics_Success() throws Exception {
        when(subTopicService.getSubTopicById(anyInt())).thenReturn(new SubTopicResponse());
